...
def create_input_form_2():
    ...
    field_info = [
        ("Term Name", "Fall 2025"),
        ("Course Search", "202510-PHL-113-A"),
        ("Student Search", "jsmith"),
    ...
    def generate_report():
        def norm(v):
            return v.strip().lower() if v not in (None, "") else None
        try:
            ...
            for k, (var, ph) in entry_vars.items():
                ...
                nv = norm(v)
                nph = norm(ph)
                values[k] = None if nv == "" or nv == nph else nv

            term_id = process_input.process_term_name(values.get("Term Name"))

            # Flatten into the same structure your old function used
            input_data = [
                term_id,
                values.get("Course Search"),
                values.get("Student Search"),
            ...
            populate_cache.call_populate(input_data=input_data)
